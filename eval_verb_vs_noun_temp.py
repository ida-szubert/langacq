...
def main(args):
    if len(args) != 5:
        print('Usage eval_verb_vs_noun.py <files : delimited> <thresh> <output file> <T to display ratio, abs size otherwise>')
        sys.exit(-1)
    thresh = float(args[2])
    output_file = args[3]
    ratio = (args[4] == 'T')
    ...
    print(zip(sc_verbs, ratio_verbs))
    print(zip(sc_nouns, ratio_nouns))
